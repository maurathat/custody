...
This follows the structure of the chain in SPEC §5, with the same agents, relations and substitution, but every digest here is computed from real payloads; the digests printed in SPEC §5 are illustrative placeholders. [`chain.json`](chain.json) holds the records and the exact wire payloads their receivers got. `impl/verify.py` recomputes every digest below from those bytes and rebuilds the graph. `impl/build_vectors.py` generates everything in the tables; none of it is typed by hand.
...
| seq 2 → seq 1 | **Failed** | `H2_VERBATIM_BYTES_CHANGED`, `H3_SUBJECT_DISCONTINUITY` |
...
The second record is the case the same machinery must *not* flag. `subject` and `observed` sit in different digest contexts: H3 checks the first against the predecessor, and H2 checks the second. A reader who trusted `observed` alone would call the second hop tampering. A reader who trusted `rel` alone would miss the third.
